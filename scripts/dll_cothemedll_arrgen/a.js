...

{
    ...
    for (let [i, byte] of tableStr.split(/\s+/g).entries())
    {
        ...
        // currNum |= parseInt(byte, 16) << (8 * (5 - (i % 6)));

        // console.log(currNum.toString(16).padStart(12, "0"))
...
[
    ...
    MUSIC  .reduce((acc, e) => Math.max(acc, e.length), 0),
...
let lines = (new Array(235)).fill("")
    .map((val, idx) => {
        let out = "";
        out += `/* ${idx.toString().padStart(3, " ")} */`;
        out += ` { `;
        out += (MAP[tableArr[idx].map - 0x00A0] + ",")    .padEnd(maxStrLens[0] + 2, " ");
        out += (MUSIC[tableArr[idx].music - 0x0000] + ",").padEnd(maxStrLens[1] + 2, " ");
        out += (tableArr[idx].unk03 + ", ").padStart(2, " ");
        out += ("0x" + tableArr[idx].instrumentSet.toString(16).toUpperCase()).padStart(4, "0");
        out += ` },`;
        return out;
    }).join("\n")
