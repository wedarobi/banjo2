...
let tableStr = "00 e5 00 e4 01 23 01 0a 00 f9 00 d1 00 fc 01 10 01 41 01 5b 01 5e 01 7a 01 13 00 c6 00 c8 00 c9 00 de 00 df 00 e0 01 2e 01 2f 01 30 01 9a 01 2b 01 2c 01 3f";
...

{
    ...
    {
        currNum |= parseInt(byte, 16) << (8 * (1 - (i % 2)));
...
let lines = (new Array(tableStr.split(/\s+/g).length / 2)).fill("")
    .map((val, idx) => {
        let out = "";
        out += `/* ${idx.toString().padStart(3, " ")} */`;
        out += ` { `;
        out += (MAP[tableArr[idx] - 0x00A0] + ",")              .padEnd(maxStrLens[0] + 2, " ");
        out += ` },`;
        return out;
    }).join("\n")

fs.writeFileSync("./out_c.txt", lines);
